...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        // Ensure Nurses table exists (in case migration didn't create it)
        try
        {
            // Check if Nurses table exists by attempting to query it
            _ = await db.Database.ExecuteSqlRawAsync(
                ...
                "UNIQUE (UserId))");
            Log.Information("Nurses table verification completed");
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error ensuring Nurses table exists");
        }
    }
    catch (Exception ex)
    {
    ...
    var roles = new[] { "Doctor", "Patient", "Nurse" };
