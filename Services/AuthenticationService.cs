    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<AuthResponse> RegisterAsync(RegisterRequest request)
        {
            ...
            try
            {
                ...
                {
                    await CreateNurseProfileAsync(user.UserId);
                }
        ...
        private async Task CreateNurseProfileAsync(int userId)
        {
            try
            {
                // For small clinic: assign nurse to the first/only doctor if exists
                var doctor = await _context.Doctors.FirstOrDefaultAsync();
                
                var nurse = new NurseModel
                {
                    UserId = userId,
                    DoctorId = doctor?.DoctorId, // Assign to doctor if exists, otherwise null
                    Department = "General",
                    CreatedAt = DateTime.UtcNow,
                    UpdatedAt = DateTime.UtcNow
                };
                ...
                if (doctor != null)
                {
                    _logger.LogInformation("Nurse profile created for UserId: {UserId}, NurseId: {NurseId}, assigned to DoctorId: {DoctorId}", 
                        userId, nurse.NurseId, doctor.DoctorId);
                }
                else
                {
                    _logger.LogInformation("Nurse profile created for UserId: {UserId}, NurseId: {NurseId} (no doctor assigned yet)", 
                        userId, nurse.NurseId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to create nurse profile for UserId: {UserId}", userId);
                throw;
            }
        }
        ...
        {
            var roles = new[] { "Doctor", "Patient", "Nurse", "Admin" };
